...
BEGIN
  ...
  -- In production, this will be your Supabase project URL + /functions/v1/generate-weekly-recap
  -- The Edge Function URL should be configured as a secret or retrieved dynamically
  edge_function_url := current_setting('app.settings.edge_function_base_url', true) || '/functions/v1/generate-weekly-recap';
  service_role_key := current_setting('app.settings.service_role_key', true);

  -- If settings are not configured, try using the Supabase env
  IF edge_function_url IS NULL OR edge_function_url = '/functions/v1/generate-weekly-recap' THEN
    -- Fallback: construct URL from Supabase project ref
    -- Note: In production, you should set app.settings.edge_function_base_url
    RAISE NOTICE 'Edge function URL not configured. Please set app.settings.edge_function_base_url';
    RETURN;
  END IF;
