...
import type { WeeklyRecap, RecapData } from '@cooked/shared';
...
export function useRecaps(): UseRecapsReturn {
  ...
  const fetchRecap = useCallback(async (recapId: string): Promise<WeeklyRecap | null> => {
    setIsLoading(true);
    setError(null);
    try {
      ...
      console.error('Fetch recap exception:', e);
  ...
  const fetchRecapHistory = useCallback(async (groupId: string, limit = 10): Promise<WeeklyRecap[]> => {
    setIsLoading(true);
    setError(null);
    try {
      const { data, error: fetchError } = await supabase
      ...
      console.error('Fetch recap history exception:', e);
      setError('Something went wrong');
      return [];
    } finally {
      setIsLoading(false);
  ...
  return { isLoading, error, fetchRecap, fetchRecapHistory };
}
